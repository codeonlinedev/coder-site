    ...
    },
    {
      services: {
        ...
        addDescription: (context) => {
          const { createWorkspaceRequest, workspace} = context
          if (!createWorkspaceRequest || !workspace) {
            throw new Error("No create workspace request || No workspace data")
          }
          return addDescriptionWorkspace(createWorkspaceRequest.description, workspace.id);
        },
      },
      guards: {
